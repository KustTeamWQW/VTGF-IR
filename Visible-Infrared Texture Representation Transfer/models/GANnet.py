...
class DiscriminatorForVGG(nn.Module):
    def __init__(
        ...
        self.features = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, True),
            # state size. (128) x 96 x 96
            nn.Conv2d(int(2 * channels), int(2 * channels), (3, 3), (1, 1), (1, 1), bias=False),
            nn.BatchNorm2d(int(2 * channels)),
            nn.LeakyReLU(0.2, True),
            # state size. (128) x 96 x 96
            ...
            nn.LeakyReLU(0.2, True),
            # state size. (256) x 96 x 96
            nn.Conv2d(int(4 * channels), int(4 * channels), (3, 3), (1, 1), (1, 1), bias=False),
            nn.BatchNorm2d(int(4 * channels)),
            nn.LeakyReLU(0.2, True),
            # state size. (256) x 96 x 96
        ...
        self.classifier = nn.Conv2d(int(8 * channels), out_channels, (1, 1), (1, 1), (0, 0), bias=True)

    def forward(self, x: Tensor) -> Tensor:
        # Input image size must equal 96
        assert x.size(2) == 300 and x.size(3) == 300, "Input image size must be 96x96"
