...
def main():
    ...
    for epoch in range(start_epoch, config["TRAIN"]["HYP"]["EPOCHS"]):
        ...
        #                 is_best,
        #                 is_last)
        save_interval = 5000  # 设置保存间隔
        if (epoch + 1) % save_interval == 0 or is_best or is_last:
            save_checkpoint({
                "epoch": epoch + 1,
                "psnr": psnr,
                "ssim": ssim,
                ...
                "ema_state_dict": ema_g_model.state_dict() if ema_g_model is not None else None,
                "optimizer": g_optimizer.state_dict()
            }, f"epoch_{epoch + 1}.pth.tar", samples_dir, results_dir, "g_best.pth.tar", "g_last.pth.tar", is_best,
                is_last)
            save_checkpoint({
                "epoch": epoch + 1,
                "psnr": psnr,
                ...
                "state_dict": d_model.state_dict(),
                "optimizer": d_optimizer.state_dict()
            }, f"epoch_{epoch + 1}.pth.tar", samples_dir, results_dir, "d_best.pth.tar", "d_last.pth.tar", is_best,
                is_last)
